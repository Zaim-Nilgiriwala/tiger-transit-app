// =============================================================================
// REFERENCE: ETASpot GTFS-RT Service (extracted from backend before purge)
//
// This file preserves the working protobuf feed parsing logic, data model,
// and trip update processing. Use this as a reference when building the
// Supabase-backed ingestion service.
// =============================================================================

// --- FEED URLs ---
// Position updates (vehicle locations):
//   https://s3.amazonaws.com/etatransit.gtfs/auburn.etaspot.net/position_updates.pb
// Trip updates (ETAs, delays):
//   https://s3.amazonaws.com/etatransit.gtfs/auburn.etaspot.net/trip_updates.pb
// Poll interval: 5000ms

// --- DATA MODEL ---
// VehiclePosition: the enriched object combining position + trip update data
//
// interface VehiclePosition {
//   vehicleId: string;    // from vp.vehicle.id or entity.id
//   routeId: string;      // from vp.trip.routeId
//   lat: number;          // from vp.position.latitude
//   lon: number;          // from vp.position.longitude
//   heading: number;      // from vp.position.bearing (0 if missing)
//   speed: number;        // from vp.position.speed (0 if missing)
//   nextStopId: string;   // from vp.stopId OR trip update's next stop
//   etaSeconds: number;   // seconds until arrival at next stop (from trip updates)
//   onTime: number;       // 1 if delay <= 0, else 0
//   isDelayed: boolean;   // true if delay > 300 seconds (5 min)
//   timestamp: number;    // vp.timestamp * 1000 (converted to ms)
// }
//
// TripEta: intermediate object from trip update processing
//
// interface TripEta {
//   nextStopId: string;
//   etaSeconds: number;
//   delay: number;        // seconds of delay from trip update
// }

// --- KEY LOGIC ---

// 1. FETCHING: Both feeds are protobuf (gtfs-realtime-bindings)
//    const buffer = await fetch(url).then(r => r.arrayBuffer());
//    const feed = transit_realtime.FeedMessage.decode(new Uint8Array(buffer));

// 2. TRIP UPDATES (process FIRST so ETAs are available for positions):
//    - Iterate feed.entity, get entity.tripUpdate
//    - For each tripUpdate, find the first stopTimeUpdate where arrival.time > now
//    - Store { nextStopId, etaSeconds, delay } keyed by tripId

// 3. POSITION UPDATES:
//    - Iterate feed.entity, get entity.vehicle
//    - Skip if no position or no routeId
//    - Look up tripEta by vp.trip.tripId to enrich with ETA/delay data
//    - nextStopId fallback chain: vp.stopId -> tripEta.nextStopId -> ''

// 4. STALE FILTERING: Vehicles older than 2 minutes are excluded from getVehicles()
